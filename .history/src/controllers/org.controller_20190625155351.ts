...
export class OrgController {
  ...
  @get('/orgs/text/{text}')
  async findOrgTexts(@param.path.string('text') filterQ: string): Promise<Org[]> {
    var searchQ = [{ name: { "regex": filterQ.toString(), "options": "si" } },
    { description: { "regex": filterQ.toString(), "options": "si" } }
    ];
    return await this.orgRepo.find({ where: { or: searchQ } });
  }
  ...
  @del('/orgs/{id}')
  async deleteOrg(@param.path.string('id') id: string): Promise<void> {
